(() => {
  ...
  const sessionRow = (session, {chip = false, activity = false} = {}) => {
    ...
    const stacked = chip || activity;
    const identity = `<span class="status-dot ${dotClass(session.status)}" aria-hidden="true"></span><span class="swatch" style="background:${esc(session.color)}" title="Session color identifier" aria-hidden="true"></span><span class="session-main"><span class="session-name">${esc(session.title)}</span></span>`;
    const state = `<span class="session-state ${esc(session.status)}" title="Status: ${esc(session.status)}" aria-label="Status: ${esc(session.status)}"><span aria-hidden="true">${statusGlyph(session.status)}</span></span>`;
    const count = `<span class="badge blue" title="${artifactCount} artifacts" aria-label="${artifactCount} artifacts">◆ ${artifactCount}</span>`;
    // Recent/pinned rows stack identity above metadata like production SessionList
    // because the narrow sidebar cannot fit chip + activity + target inline.
    const body = stacked
      ? `<span class="row-line1">${identity}</span><span class="row-line2"><span class="badge chip" title="Project: ${esc(session.project)}">${esc(session.project)}</span><span class="activity" title="${esc(act.label)}" aria-hidden="true">${esc(act.label)}</span><span class="badge">${esc(session.target)}</span>${count}${state}</span>`
      : `${identity}<span class="badge">${esc(session.target)}</span>${state}${count}`;
    return `<div class="session${stacked ? ' stacked' : ''}${selected ? ' selected' : ''}" data-title="${esc(session.title)}" data-project="${esc(session.project)}" data-branch="${esc(session.branch)}" data-status="${esc(session.status)}" data-activity="${session.activityAt ?? ''}" data-pinned="${String(!!session.pinned)}" data-detail="${esc(detail)}"><button class="session-select${stacked ? ' stacked' : ''}" title="${esc(`${session.title} — ${detail}`)}" aria-label="${esc(`${session.title}, ${session.status}, ${session.target} target, ${artifactCount} artifacts, ${session.project} project, ${act.label}`)}"${selected ? ' aria-current="true"' : ''}>${body}</button><button class="session-pin" data-pin-branch="${esc(session.branch)}" aria-pressed="${String(!!session.pinned)}" aria-label="${esc(`${session.pinned ? 'Unpin' : 'Pin'} ${session.title}`)}" title="${session.pinned ? 'unpin session' : 'pin session'}"><span aria-hidden="true">${session.pinned ? '●' : '○'}</span></button></div>`;
  };
